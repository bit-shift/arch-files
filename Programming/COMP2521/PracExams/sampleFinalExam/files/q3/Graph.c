...
Connects *wellConnected(Graph g, int *nconns)
{
   ...
   *nconns = 0;
   for (int i = 0; i < g->nV; i++) {
       Connects *insert = malloc(sizeof(Connects));
       insert->vertx = i;
       insert->nconn = 0;
       ...
       if (insert->nconn > 1) {
           well[*nconns].vertx = i;
           well[*nconns].nconn = insert->nconn;
           *nconns = *nconns+1;
       }
       free(insert);
   }
   //Sort this bad boy
   for (int u = 1; u < *nconns; u++) {
       for (int v = u; v > 0; v--) {
           if (well[v].nconn < well[v-1].nconn || (well[v].nconn == well[v-1].nconn && well[v].vertx < well[v-1].vertx)) {
               Connects temp = well[v-1];
               well[v-1] = well[v];
               well[v] = temp;
           }
           else break;
