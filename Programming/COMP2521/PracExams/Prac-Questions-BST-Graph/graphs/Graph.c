...
int hasCycle(Graph g) {
    ...
    for (int i = 0; i < g->nV; i++) {
        ...
        while (!emptyQueue(todo)) {
            Vertex u = leaveQueue(todo);
            if (visited[u]) return 1; 
            visited[u] = 1;
            for (int v = 0; v < g->nV; v++) {
                if (adjacent(g, u, v) && v != pred[u]) {
                    enterQueue(todo, v);
                    pred[v] = u;
                }
            }
...
int* within(Graph g, int s, int d, int *size) {
    ...
    int *visited = malloc(sizeof(int)*g->nV);
    int *within = malloc(sizeof(int)*g->nV);
    int count = 0;
    for (int i = 0; i < g->nV; i++) {
        dist[i] = 0;
        visited[i] = 0;
    ...
    while (!emptyQueue(todo)) {
        Vertex u = leaveQueue(todo);
        visited[u] = 1;
        for (int v = g->nV-1; v >= 0; v--) {
            if (adjacent(g, u, v) && !visited[v]) {
                enterQueue(todo, v);
                dist[v] = dist[u]+1;
                if (dist[v] <= d) within[count++] = v;
            }
        }
    }
    *size = count;
...
int *components(Graph g) {
    int compNo = 0;
    int *component = malloc(sizeof(int)*g->nV);
    int *visited = malloc(sizeof(int)*g->nV);
    for (int i = 0; i < g->nV; i++) {
        component[i] = -1;
        visited[i] = 0;
...
int bipartition(Graph g, List l1, List l2) {
    int *redBlue = malloc(sizeof(int)*g->nV);
    int *visited = malloc(sizeof(int)*g->nV);
    for (int i = 0; i < g->nV; i++) {
        redBlue[i] = -1;
        visited[i] = 0;
        ...
        while (!emptyQueue(todo)) {
            ...
            for (int v = 0; v < g->nV; v++) {
                if (adjacent(g, u, v)) {
                    if (!visited[v]) {
                        enterQueue(todo, v);
                        redBlue[v] = (redBlue[u])? 0 : 1;
                    }
                    else {
                        if(redBlue[v] == redBlue[u]) return 0;
                    }
                }
            }
        }
    ...
    for (int i = 0; i < g->nV; i++) {
        if (redBlue[i]) ListInsert(l1, i);
        else ListInsert(l2, i);
    }
    return 1;
}
