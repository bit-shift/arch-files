...
int findValue(Tree t, int v) {
    if (t == NULL) return 0;
    else if (v < t->val) return findValue(t->left, v);
    else if (v > t->val) return findValue(t->right, v);
    else return 1;
}
...
int isBST(Tree t, Tree l, Tree r) {
    if (t == NULL) return 1;
    
    if (l != NULL && t->val < l->val) return 0;
    if (r != NULL && t->val > r->val) return 0;

    return isBST(t->left, l, t) && isBST(t->right, t, r);
}
...
void getKthSmallest(Tree t, int* k, int** v) {
    ...
    for (int i = 0; i < *k; i++) succ = successor(t, succ);
    *v = &(succ->val);
}
...
Tree lowestCommonAnc(Tree t, int v1, int v2) {
    ...
    if (t->left == NULL) {
        ...
        v1Check = (findValue(t->left, v1)) ? 1 : 0;
        v2Check = (findValue(t->left, v2)) ? 1 : 0;
    }
    if ((v1Check && !v2Check) || (!v1Check && v2Check)) return t;
    else if (v1Check && v2Check) return lowestCommonAnc(t->left, v1, v2);
    else if (!v1Check && !v2Check) return lowestCommonAnc(t->right, v1, v2);
    else return NULL;
}
