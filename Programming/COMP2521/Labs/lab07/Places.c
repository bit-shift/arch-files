...
int nameToID(char *name, int lo, int hi)
{
   int mid = (lo+hi)/2;
   if (lo > hi) return NOWHERE;
   if (strcmp(name, places[mid].name) == 0) return places[mid].id;
   else if (strcmp(name, places[mid].name) < 0) return nameToID(name, lo, mid-1);
   else return nameToID(name, mid+1, hi);
}
